...
private final class TerminalInputField: UITextField {
  ...
  private static let hardwareKeyCommands = TerminalHardwareKeyEncoder.makeKeyCommands(
    action: Selector("handleHardwareKeyCommand")
  )
...
public final class T3TerminalView: ExpoView, UITextFieldDelegate {
  ...
  public required init(appContext: AppContext? = nil) {
    ...
    scrollPanGesture.addTarget(self, action: Selector("handleViewportPan"))
...
extension TerminalInputField: T3ActionPerformable {
  public func t3Perform(_ action: Selector, _ argument: Any?) {
    switch action.t3Name {
    case "handleHardwareKeyCommand": handleHardwareKeyCommand(argument as! UIKeyCommand)
    default: break
    }
  }
}
...
// —— Generated by sync-upstream.mjs: selector-free 动作分发表 ——
extension T3TerminalView: T3ActionPerformable {
  public func t3Perform(_ action: Selector, _ argument: Any?) {
    switch action.t3Name {
    case "handleInputEditingDidBegin": handleInputEditingDidBegin()
    case "handleViewportTap": handleViewportTap()
    case "handleViewportPan": handleViewportPan(argument as! UIPanGestureRecognizer)
    default: break
    }
  }
}
