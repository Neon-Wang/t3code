...
private final class ReviewDiffContentView: UIView, UIGestureRecognizerDelegate {
  ...
  private lazy var horizontalPanGesture: UIPanGestureRecognizer = {
    let gesture = UIPanGestureRecognizer(target: self, action: Selector("handleHorizontalPan"))
  ...
  private func handleHorizontalPan(_ gesture: UIPanGestureRecognizer) {
  ...
  private func startHorizontalDeceleration(fileId: String, velocity: CGFloat) {
    ...
    let displayLink = CADisplayLink(target: self, selector: Selector("stepHorizontalDeceleration"))
    displayLink.add(to: .main, forMode: .common)
    decelerationDisplayLink = displayLink
  }

  private func stepHorizontalDeceleration(_ displayLink: CADisplayLink) {
...
extension T3ReviewDiffView: T3ActionPerformable {
  public func t3Perform(_ action: Selector, _ argument: Any?) {
    switch action.t3Name {
    case "handlePullToRefresh": handlePullToRefresh()
    default: break
    }
  }
}
...
// —— Generated by sync-upstream.mjs: selector-free 动作分发表 ——
extension ReviewDiffContentView: T3ActionPerformable {
  public func t3Perform(_ action: Selector, _ argument: Any?) {
    switch action.t3Name {
    case "handleHorizontalPan": handleHorizontalPan(argument as! UIPanGestureRecognizer)
    case "handleTap": handleTap(argument as! UITapGestureRecognizer)
    case "handleLongPress": handleLongPress(argument as! UILongPressGestureRecognizer)
    case "stepHorizontalDeceleration": stepHorizontalDeceleration(argument as! CADisplayLink)
    default: break
    }
  }
}
