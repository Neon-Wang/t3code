...
private final class ComposerTextAttachment: NSTextAttachment {
  ...
  init(source: String, image: UIImage, size: CGSize, baselineOffset: CGFloat) {
    self.source = source
    super.init(attachmentPlaceholder: ())
    self.image = image
...
private final class ComposerTextView: UITextView {
  ...
  private func submitMessage(_ sender: UIKeyCommand) {
    onSubmit?()
  }
  ...
  func serializedText(in range: NSRange) -> String {
    ...
    let source = NSMutableString(capacity: 0)
...
extension ComposerTextView: T3ActionPerformable {
  public func t3Perform(_ action: Selector, _ argument: Any?) {
    switch action.t3Name {
    case "submitMessage": submitMessage(argument as! UIKeyCommand)
    case "paste": paste(argument as! Any?)
    default: break
    }
  }
}
